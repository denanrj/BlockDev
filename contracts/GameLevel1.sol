...
contract LiskGarden {
    ...
    function calculateWaterLevel(uint256 plantId) public view returns (uint8) {
        ...
        if(plant.isDead == true || plant.exists == false){
            return 0;
        }
    ...
    function waterPlant(uint256 plantId) external {
        // TODO: Implement
        Plant storage plant = plants[plantId];
        require(plant.exists == true, "Plant Tidak Tersedia!");
        require(plant.owner == msg.sender, "Bukan Tanaman Anda!");
        require(plant.isDead == false, "Tanaman Tidak Hidup!");
    ...
    function updatePlantStage(uint256 plantId) public {
        ...
        if (timeSincePlanted >= 3 * STAGE_DURATION && oldStage != GrowthStage.BLOOMING) {
        ...
        } else if (timeSincePlanted >= 1 * STAGE_DURATION && plant.stage != GrowthStage.SEED) {
    ...
    function harvestPlant(uint256 plantId) external {
        ...
        updatePlantStage(plantId);
        require(plant.stage == GrowthStage.BLOOMING, "Tanaman Harus Berbunga");
        plant.exists = false;
